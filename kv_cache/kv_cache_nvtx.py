...
class KVCache:
    @nvtx.annotate("kv_cache_init", color="green")
    def __init__(
            self,
            max_batch_size: int,
            max_seq_len: int,
        ...
        # 标记内存分配部分（初始化分配内存）
        with nvtx.annotate("kv_cache_memory_alloc", color="orange"):
    ...
    @nvtx.annotate("kv_cache_update", color="blue")
    def update(
            self,
            batch_idx: int,
            start_pos: int,
        ...
        # 标记缓存写入操作
        with nvtx.annotate("kv_cache_write_ops", color="red"):
        ...
        # 标记缓存读取和返回操作
        with nvtx.annotate("kv_cache_read_return", color="purple"):
            return (
                self.K_cache[batch_idx, :, :end_pos, :].transpose(0, 1),
                self.V_cache[batch_idx, :, :end_pos, :].transpose(0, 1)
            )

    @nvtx.annotate("kv_cache_get", color="cyan")
    def get(
            self,
            batch_idx: int
    ) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    @nvtx.annotate("kv_cache_clear", color="gray")
    def clear(
            self,
            batch_idx: int | None
    ):
